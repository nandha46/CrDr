...
--
-- Host: 127.0.0.1
-- Generation Time: Nov 22, 2022 at 03:06 PM
...
CREATE ALGORITHM = UNDEFINED DEFINER = `root` @`localhost` SQL SECURITY DEFINER VIEW `view_students` AS 
SELECT 
  `a`.`stuId` AS `id`, 
  `a`.`stuFirstName` AS `firstname`, 
  `a`.`stuLastName` AS `lastname`, 
  if(
    `a`.`stuIsStatus` = 1, 'Active', 'In-Active'
  ) AS `studentstatus`, 
  `a`.`stuUserId` AS `student_userid`, 
  `a`.`stuIsStatus` AS `status`, 
  `a`.`stuGender` AS `gender`, 
  `a`.`stuDOB` AS `dob`, 
  `b`.`orstOrgMasterId` AS `orgnId`, 
  `c`.`stuclOrgStuMasterId` AS `OrgStuId`, 
  `c`.`stuclClsMasterId` AS `ClassId`, 
  `c`.`stuclSecMasterId` AS `SectionId`, 
  `c`.`stuclAcademicYear` AS `academicYear`, 
  `c`.`promoted` AS `promoted`, 
  `c`.`transferred` AS `transferred`, 
  `d`.`olName` AS `branchName`, 
  `e`.`secnSecName` AS `secnName`, 
  `f`.`clssName` AS `clssName`, 
  `i`.`condOfficialNumber` AS `OfficialNumber`, 
  `i`.`condPersonalNumber` AS `PersonalNumber`, 
  `j`.`emlOfficialAddress` AS `email`, 
  `k`.`address` AS `address`, 
  `k`.`city` AS `city`, 
  `k`.`state` AS `state`, 
  `m`.`statename` AS `state_name`, 
  `k`.`pincode` AS `pincode`, 
  `l`.`id_picture` AS `profile_pic`, 
                ...
                (
                  (
                    (
                      (
                        `students` `a` 
                        left join `orgstudents` `b` on(
                          `a`.`stuId` = `b`.`orstStuMasterId`
                        )
                      ) 
                      left join `studentsclasses` `c` on(
                        `b`.`orstId` = `c`.`stuclOrgStuMasterId`
                      )
                    ) 
                    left join `orgdetails` `d` on(
                      `d`.`olSubOrgId` = `b`.`branch_id`
                    )
                  ) 
                  left join `sections` `e` on(
                    `e`.`secnId` = `c`.`stuclSecMasterId`
                  )
                ) 
                left join `classes` `f` on(
                  `c`.`stuclClsMasterId` = `f`.`clssId`
                )
              ) 
              left join `family` `g` on(
                `a`.`stuUserId` = `g`.`userid`
              )
            ) 
            left join `contactdetails` `i` on(`i`.`condId` = `g`.`contact_id`)
          ) 
          left join `emailaddresses` `j` on(`j`.`emlId` = `g`.`email_id`)
        ) 
        left join `addresses` `k` on(`k`.`id` = `g`.`address_id`)
      ) 
      left join `users` `l` on(`l`.`id` = `a`.`stuUserId`)
    ) 
    left join `states` `m` on(`m`.`id` = `k`.`state`)
  );
